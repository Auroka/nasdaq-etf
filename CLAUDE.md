...
- 历史 ETF 补录优先使用估值日记 T-1/IOPV；目标估值日已不在页面时，使用东方财富基金历史单位净值兜底。
...
- ETF 分钟走势按东方财富、腾讯证券当天分时、新浪财经分钟 K 线的顺序获取；QQQ/NDX 分钟走势先用 Nasdaq，失败后用 Yahoo Finance 兜底。
- ETF 分钟走势必须覆盖开盘到收盘，最终点必须与表格的 `15:00` 收盘价一致；不完整的新浪走势不能写入。
