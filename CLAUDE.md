...
| 估值（净值） | `westock_etf_detail` (腾讯自选股 ETF 详情 nav 字段) | 无，缺失则失败 |
| 溢价率 | 按 `price / nav - 1` 计算 | — |
| 分钟走势 | `westock_minute` (腾讯自选股分时) | `eastmoney_trend` → `tencent_minute` → `sina_minute_kline` → `yahoo_intraday_chart` |

### ETF 历史补录
| 数据 | 顺序 |
| --- | --- |
| 历史收盘和涨幅 | `westock_kline` → `eastmoney_kline` → `sina_daily_kline` |
| 估值（净值） | `westock_etf_detail`，记录写入时刻的最新可用净值 |
...
- 历史 ETF 价格和涨幅优先使用腾讯自选股历史 K 线，不可用时用东方财富，再不可用时用新浪兜底。
...
- ETF 分钟走势优先使用腾讯自选股分时，兜底用东方财富、腾讯证券分时、新浪分钟 K 线；QQQ/NDX 分钟走势优先用腾讯自选股分时，兜底用 Nasdaq 和 Yahoo Finance。
- ETF 分钟走势必须覆盖开盘到收盘，最终点必须与表格的 `15:00` 收盘价一致。
- QQQ/NDX 分钟走势只保留美股常规交易时段 `9:30 AM` 到 `4:00 PM`；最终点必须与表格使用的官方收盘价一致。
